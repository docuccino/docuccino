...
A regeneration obeys that rule from the write side too: it puts the freshly generated bytes down with
the version each golden already records, so a golden whose content did not move does not move at all
and the isolated commit stays isolated. A golden with no recorded version — a new one, or a file too
mangled to read one out of — takes the current version, which is also how you deliberately re-record
one: delete it and regenerate.
