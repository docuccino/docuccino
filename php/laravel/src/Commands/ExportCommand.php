...
final class ExportCommand extends Command
{
    ...
    private function write(ExportTarget $target, UirDocument $document, DocumentConfig $config): bool
    {
        ...
        // An emitter that reports an ERROR has written a file that is not a valid document of the format
        // it claims, so the run failed however loud the reader asked diagnostics to be. `--fail-on` is
        // how strict you want to be about what the document SAYS; a malformed artifact is not that
        // question, and `docuccino:validate` already treats the UIR half the same way.
        return array_filter(
