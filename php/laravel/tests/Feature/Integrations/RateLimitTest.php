 ...
 * for every throttled route whatever its limit; a named limiter it cannot read still raises an info
 * diagnostic.
 */
...
it('documents a named limiter it cannot read with the same 429, plus an info diagnostic', function (): void {
    /** @var Router $router */
    $router = app('router');
...
it('reports nothing for a registered named limiter it can read statically', function (): void {
    ...
    $result = generateDocument();
    $document = $result->document->toArray();
    $operation = $document['paths']['/api/folded-throttle']['get'] ?? [];

    // A readable limiter documents exactly what an unreadable one does — the fold only decides whether
