...
function publicExtensionSurface(): array
{
    return [
        ...
        // route can name it by — the registered alias, or the class name `Authenticate::using()` renders.
        // Whether a route is authenticated is one fact with readers on both sides of the line: the
        // security layer and the implicit 401 are extensions, Sanctum's mode detection and the
        // guard→driver resolution are integrations. Each holding its own list is how three of them came
        // to match the alias only, and to publish an authenticated route as public.
