...
final class RouteReflector
{
    ...
    private function splitAction(string $uses): array
    {
        if (str_contains($uses, '@')) {
            ...
            return [class_exists($class) ? $class : null, $method];
        }

        // A single-action controller referenced by class name resolves through __invoke.
        return [class_exists($uses) ? $uses : null, '__invoke'];
    }
}
