...
3. **Replace the mapper.** Register your own `ExceptionToResponse` for the exception. The chain takes
   ...
   tiers still run ahead of it: the inferred-handler tier (`FIRST`), which documents the shape your
   app's own handler really returns, and the Problem Details preset (`EARLY`) where you've opted in.
   To beat either, say so — `#[ExtensionOrder(priority: Priorities::FIRST + 1)]`.
