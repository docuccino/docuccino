...
| | |
| --- | --- |
| Legal names | `^[a-zA-Z0-9._-]+$`. Anything else is refused at the call — read as no name at all, so the body falls back to `Error<status>` and the document is never emitted invalid. |
...
3. **Keep the body, rename the component.** An `OperationExtension` in the `Finalize` phase claims a
   ...
   [precedence](#every-write-carries-a-contribution) decides — and the built-in tiers claim at
   `integration`, the same layer your extension writes at. An equal contribution is shadowed, so break
   the tie the way every other field breaks one, with specificity:
...
one of the two paths above — see [Overlays](/laravel/guides/customizing-output/) for what overlays are
for.
:::
