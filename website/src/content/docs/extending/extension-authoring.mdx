...
from whichever class happened to throw.
...
| | |
| --- | --- |
| Legal names | `^[a-zA-Z0-9._-]+$`. Anything else is refused with a `components.name-invalid` warning naming your producer, and the body falls back to `Error<status>` — the document is never emitted invalid. |
...
| Only bodies that repeat | A body one operation states stays inline and is never named at all, claim or no claim. |

#### Overriding a built-in name

Docuccino's own tiers name Laravel's errors `NotFound`, `Unauthorized`, `Forbidden`,
`UnprocessableEntity`, `TooManyRequests` and `BadRequest`. Two supported ways to change that, depending
on whether you want a different body too.

<Steps>

1. **Replace the mapper.** Register your own `ExceptionToResponse` for the exception. The chain takes
...
2. **Keep the body, rename the component.** An `OperationExtension` in the `Finalize` phase claims a
