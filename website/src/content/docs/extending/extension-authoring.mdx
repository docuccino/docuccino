...
Docuccino's own tiers name Laravel's errors `NotFound`, `Unauthorized`, `Forbidden`,
`UnprocessableEntity`, `TooManyRequests` and `BadRequest`. Three supported ways to change that, in the
order to reach for them.

<Steps>

1. **Mark the exception.** For your own exception classes this is the whole job — no extension, no
   registration:
...
2. **Replace the mapper.** Register your own `ExceptionToResponse` for the exception. The chain takes
...
3. **Keep the body, rename the component.** An `OperationExtension` in the `Finalize` phase claims a
...
one of the three paths above — see [Overlays](/laravel/guides/customizing-output/) for what overlays are
for.
:::
