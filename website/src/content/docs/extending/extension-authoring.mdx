...
1. **Name the response on the operation that declares it.** Where the action declares the status itself
   with `#[Response]`, its `component:` argument names the response — no extension, and the highest of
   the four, so it stands over anything the paths below claim:

   ```php
   use Docuccino\Attributes\Response;

   #[Response(status: 422, type: SignInChallenge::class, component: 'AuthenticationChallenge')]
   ...
   Full behavior: [Name a response you declared
   yourself](/laravel/documenting/errors/#name-a-response-you-declared-yourself).
