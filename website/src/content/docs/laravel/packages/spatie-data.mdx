...
PHP can't type an array's elements, so Docuccino reads the tag that can: a promoted property's `@param`
on the constructor, a plain property's own `@var`.
...
The tag is consulted only where the declaration is vague — a bare `array`, a `mixed`, nothing at all —
and only if the tag itself is precise, so a native `string` is never second-guessed and a `@var array`
never displaces a real declaration. Short class names resolve through that file's `use` statements. The
