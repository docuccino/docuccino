...
[Request bodies](/laravel/documenting/requests/) for the full detail.
...
genuinely computed status can't be read statically, so that member stays `200` with an info diagnostic
— see [Responses](/laravel/documenting/responses/#spatie-data-success-status).
