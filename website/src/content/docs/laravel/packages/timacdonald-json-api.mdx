...
The resource becomes a `{ data: { id, type, attributes, relationships } }` document schema, hoisted into
a reusable component, plus `include` and `fields[users]` query parameters on the endpoints that return
it.
