...
The [Problem Details preset](#the-problem-details-preset-opt-in) still answers in that case: a document that
opted into it has declared that shape itself, and a handler nobody could read has not contradicted it. What
does contradict it is a handler that named a content type — that reading comes from your code, so it is
published instead, unconstrained.
