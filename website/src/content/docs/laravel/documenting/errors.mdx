...
handler is too dynamic to read statically, Docuccino records a diagnostic and moves on to the next tier.

The two halves are read separately. A handler whose **body** reads but whose status does not — a status
off `$e->getCode()`, an enum method, or an `HttpException` subclass that sets its own — still publishes
that body, under the status the framework classifies the exception as. Documenting the shape your server
actually sends matters more than the number it is filed under, and the number is the one every other tier
...
diagnostic names the callback to fix; the [Problem Details preset](#the-problem-details-preset-opt-in) is not
affected, since a document that opted into it has declared that shape itself.
...
| A response two or more operations state identically, however each illustrates or describes it | A response no other operation matches, a description-only response, or one that is already a `$ref` — the preset's `Problem*` components included |
...
Where the operations agree on one illustration it stays the media type's single `example`; where they
don't, they become an `examples` map keyed from each body's own content, so adding or removing an
endpoint never renames another one's example.
