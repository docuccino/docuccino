...
[`#[Example(name: …)]`](/laravel/documenting/examples/#illustrating-something-other-than-the-success-response), the shared response
publishes your key next to the generated ones, so annotating one route never changes what the others
publish. The exception is two operations giving one name to two different examples: only you can say
