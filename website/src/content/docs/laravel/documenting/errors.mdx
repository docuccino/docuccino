...
`application/problem+json` gets a problem document there too. Set `error_responses` to `none` to turn all
of this off.
...
| --- | --- | --- | --- |
| `error_responses` | `default` \| `none` | `default` | What to publish for the exceptions your application does not render itself. Your inferred handlers take precedence regardless; `none` also turns off the implicit responses above. |
