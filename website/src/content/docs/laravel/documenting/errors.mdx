...
Before **v0.5**, shared error components were named `Error404`, `Error422` and so on. Regenerating turns
them into the names above, and any client you generate from the document renames its error types to
match. The document's contract is unchanged — same statuses, same bodies — but a `catch (Error404)` in
hand-written client code needs updating once.
:::
...
[Name a response you declared yourself](#name-a-response-you-declared-yourself) for the way to override
that.
...
reports `attribute.error-component-unread` rather than leaving you to wonder. For a body the operation
declares with `#[Response]`, name it there.
...
**Precedence, in order:** `#[Response(component:)]` on the operation, then the name a mapper gave the body
...
#### Name a response you declared yourself

`#[ErrorComponent]` names errors where they are defined — an exception class, or the method that renders
one. A `4xx` your operation **declares**, with `#[Response]`, has no such place: nothing was thrown and
no renderer built it, so neither anchor ever sees it. Name it on the declaration:
...
representation of a status, the name is the status's — two declarations at one status naming different
components is reported as `attribute.response-component-contested` and neither is used, rather than one
winning by attribute order.
...
status default from the table above, or `Error<status>` where the status has none. A refused name never
contests a good one.
