...
The check reads the controller class **and its parents**, so one such attribute on a base controller
your API classes extend is reported against every route of every child — a screenful of warnings from a
single line. It is skipped entirely under
[`error_responses => 'none'`](/laravel/reference/configuration/#error_responses), along with the rest of
this tier.
