...
is refused rather than built from defaults (`config.not-migrated`).
...
and the throttled [`429`](/laravel/documenting/rate-limiting/) — none of them are emitted under `none`.
To drop some and keep others, leave this at `default` and name the ones you don't want with
[`#[IgnoreResponse]`](/laravel/reference/attributes/#ignoreresponse).
