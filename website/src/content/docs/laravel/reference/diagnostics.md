...
| `validation.rule-values-unread` | info | A rule's values are spread in from an expression, or passed under a name, so the values themselves can't be read — the constraint is left off and the field keeps the rules that did recover | Write the values where the rule is (`Rule::in('draft', 'live')`), or state them in an [overlay](/laravel/guides/customizing-output/#openapi-overlays-in-practice) — a partial list would make a generated client reject a value the API accepts |
