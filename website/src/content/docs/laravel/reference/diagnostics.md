...
| `eloquent.custom-date-serialization` | info | A model overrides `serializeDate()` **and** the document published one of its date attributes through that override, so that attribute's wire format isn't statically known and it's documented as a plain string. Raised against the model for a response body, and against the route for a path parameter bound on a date column. Not reported where nothing published lost a format: an override inherited by a model with no published date attribute, one whose date attributes are all hidden, or one whose date attribute an accessor publishes instead (Laravel never sends a mutated attribute through the override) | Nothing you can annotate: no attribute carries a column `format`, and a docblock type has no format to state. If clients need an exact one, state it in an [overlay](/laravel/guides/customizing-output/#openapi-overlays-in-practice) — which corrects the document, and this notice keeps naming what lost the format |
