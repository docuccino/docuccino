...
| `attribute.ignore-param-unmatched` | warning | An `#[IgnoreParam]` on an action names a parameter the operation doesn't document, so it dropped nothing and the field the author meant to hide — if it is there under another name — is still published. The message lists what the operation does document. Reported for the action's own declaration only: one inherited from a controller names a key only some of its actions take, and an action that never documented it is not a mistake | Correct the name against the list in the message, or delete the declaration. A parameter that was renamed keeps its old spelling only in the attribute; a key only some actions take belongs on the class. See [`#[IgnoreParam]`](/laravel/reference/attributes/#ignoreparam) |
