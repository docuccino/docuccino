...
| `eloquent.custom-date-serialization` | info | A model overrides `serializeDate()` **and** publishes at least one date attribute, so that attribute's wire format isn't statically known and it's documented as a plain string. An override inherited by a model with no published date attribute is not reported | Nothing you can annotate: no attribute carries a column `format`, and a docblock type has no format to state. If clients need an exact one, state it in an [overlay](/laravel/guides/customizing-output/#openapi-overlays-in-practice) — which corrects the document, and this notice keeps naming the model |
