...
| `query-builder.legacy-package-version` | info | spatie/laravel-query-builder below v7 is installed, so the sort/include allow-lists are documented as plain strings rather than value enums | Upgrade to spatie/laravel-query-builder `^7` to document the sort/include allow-lists as enums |
| `query-builder.enum-name-collision` | info | Two sort/include values would share one SDK enum member name, so distinct value-derived names were published instead | Rename one of the colliding allow-list entries so each value mints its own member name |
