...
| `validation.rule-unrecoverable` | info | A validation field's rules are a closure, a custom rule object with no `#[RuleSchema]`, or a conditional descriptor, so the field is documented from its type alone — or omitted where it has no type either. Not raised for a field a [`#[BodyParameter]`](/laravel/reference/attributes/#bodyparameter) names, or names a key inside: that declaration documents the field from a layer above the rules, so the omission this reports did not happen | Express the field with recoverable rules, or annotate the rule class with [`#[RuleSchema]`](/laravel/documenting/requests/#documenting-a-custom-rule) — or declare the field with [`#[BodyParameter]`](/laravel/reference/attributes/#bodyparameter), which documents it outright |
| `validation.rule-values-unread` | info | A rule states a value that isn't written at the rule — it comes from a call, a variable or a spread — so the constraint is left off and the field keeps the rules that did recover. Not raised for a field a [`#[BodyParameter]`](/laravel/reference/attributes/#bodyparameter) names, or names a key inside: what that field publishes is the declaration's, not the rules' | Write every value where the rule is (`Rule::in('draft', 'live')`), which is what settles it — a partial list would make a generated client reject a value the API accepts. An [overlay](/laravel/guides/customizing-output/#openapi-overlays-in-practice) corrects the document instead, and this notice keeps naming the rule |
