...
| `inference.descend-scope-narrowed` | info | You set [`engine.project_paths`](/laravel/reference/configuration/#engine), and it excludes a file your `composer.json` declares — so a call that can throw was not opened and an error behind it is documented nowhere. Names the callee, the line it is called at, and the file descent stopped short of. Never fires unset, and never for vendor code: there is nothing to widen to there | Delete `engine.project_paths` from `docuccino.yaml` to descend into every PSR-4 root you declare under `autoload`, which is the default, or add this file's directory to the list you keep |
