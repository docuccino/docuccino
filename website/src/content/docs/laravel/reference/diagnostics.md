...
| `inference.http-exception-status-unread` | info | An exception extending Symfony's `HttpException` sets a status the analyzer could not read, so the error is documented without one and a later tier classifies it | Pass a constant to `parent::__construct()`. Where the status is a constructor parameter with a default, make the constructor `private` and build the exception through static factories; where it is passed in at the `throw`, write it there as a constant |
