...
| `attribute.error-component-unread` | warning | An `#[ErrorComponent]` on an action or its controller, which names nothing: the attribute names an error where the error is defined, not where an operation answers with it | Move it to the exception class the error is raised from, or to the render method that builds the body. For a body the operation declares itself, use the `component:` argument of the [`#[Response]`](/laravel/reference/attributes/#response) that declares it |
| `attribute.response-component-contested` | warning | Two `#[Response]` declarations at one status name different components, and a response component covers every representation of a status, so neither name was used | Put `component:` on one of the declarations, or spell the same name on both |
