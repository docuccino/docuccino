...
| `query-builder.untyped-filter` | info | A filter your own code handles (`AllowedFilter::callback()`/`::custom()`) and nothing typed its value, so the parameter is documented with no type at all — a generated client takes an untyped value for it | Add `#[QueryParameter(type: 'string')]` to the filter class, or to the action, so the parameter carries a type |
