...
| `workflow.output-unresolved` | warning | A step reads a `$steps.<id>.outputs.<name>` that no earlier step of the workflow produces, so an Arazzo runner meets it as a null at run time | Check the step id and the output name, and that the step producing it declares that output and comes earlier in the order |
