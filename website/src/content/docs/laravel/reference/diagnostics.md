...
| `config.not-migrated` | error | There's no `docuccino.yaml`, and `config/docuccino.php` still holds build settings — which nothing reads. Building from defaults would have thrown away the routes, info and security you configured and produced a plausible-looking document, so the build refuses instead | Run `php artisan docuccino:install` to write `docuccino.yaml`, move those settings into it, and delete them from `config/docuccino.php` |
...
| `config.route-closure-removed` | error | `routes.closure` still holds a value. The key is gone: a closure in config has no serializable form, so an application that filled it in could never run `php artisan config:cache` | Move the predicate into a class implementing `RouteFilter` and name it under `routes.filter`, which is resolved from the container so the closure's captures become constructor dependencies — see [`routes`](/laravel/reference/configuration/#routes). A `closure` key left at `null` says nothing, so deleting the line is enough where you never used it |
