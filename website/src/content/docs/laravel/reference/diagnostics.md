...
| `config.unknown-error-responses` | warning | `error_responses` isn't a strategy Docuccino has, so the document was built as if it said `default` — every error response in it takes the framework's shape rather than whatever the value meant to ask for | Set it to `default` or `none` — see [`error_responses`](/laravel/reference/configuration/#error_responses). What your own exception handling returns is read from your code either way |
