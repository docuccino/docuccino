...
| `inference.http-exception-status-unread` | info | An exception extending Symfony's `HttpException` sets a status the analyzer could not read, so the error is documented without one and a later tier classifies it | Say the status where the exception is built, as a constant. A status defaulted on the constructor is not one — a caller may pass another, and this one passed none: pin it with `parent::__construct(409, …)` if every instance is that status, and otherwise write it at the `throw`, or in the static factory the `throw` names |
