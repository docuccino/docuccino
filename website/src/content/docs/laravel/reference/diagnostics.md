...
| `attribute.error-component-unread` | warning | An `#[ErrorComponent]` on an action or its controller, which names nothing: the attribute names an error where the error is defined, not where an operation answers with it | Move it to the exception class the error is raised from, or to the render method that builds the body. For a body the operation declares itself, use the `errorComponent:` argument of the [`#[Response]`](/laravel/reference/attributes/#response) that declares it |
