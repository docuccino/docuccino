			...
			plugins: [
				starlightLlmsTxt({
					...
					details: [
						...
						'- `docuccino:explain <route>` reads the provenance trail back for one operation: which layer won each field, what it shadowed, and the file:line it came from. It accepts a route name, an operation id or a URI (with an optional leading method), lists every match when a query is ambiguous (exit 2), and needs no export flags — it builds the document itself, where the trail is always complete.',
					].join('\n'),
