...
| `operation-operationId-unique` | 3 | Two operations share an `operationId`. Reported twice: `route.duplicate-operation-id` where the second is met, naming both routes — a better vantage point than the finished document has — and `document.duplicate-operation-id` against the emitted artifact, which no meta-schema could state — a warning, because unlike the rest of that check the cause is usually the application's. A minted id is unique by construction, so a collision is either two names somebody pinned or the `controller-method` strategy over routes sharing an action. |
